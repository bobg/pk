...
//
// 1. A struct field whose type is []T or [...]T
//    becomes a slice of blobrefs of the recursively marshaled members of the slice or array.
// 2. A struct field whose type is map[K]T becomes a map[K]blob.Ref,
//    mapping each key to the blobref of the recursively marshaled value.
// 3. Any other struct-field value is recursively marshaled
//    and its blobref used in the map[string]interface{}.
//
// These rules may be overridden with struct tags using the key "pk", as follows:
//
// - `pk:"-"` means skip this field;
// - `pk:"name"` means use "name" as the field name in the map[string]interface{}
//   rather than the struct field's name;
...
// - inline, causes the field's value to be used directly in the map[string]interface{}
//   rather than recursively marshaling it;
// - external, causes container types (slices, arrays, and maps) to be marshaled separately
//   from the struct, and the resulting blobref used as the value,
//   rather than marshaling them as slices or maps of member blobrefs.
